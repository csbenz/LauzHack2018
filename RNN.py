...
def date_labels(market_data,X_test):
...
def plot_results(history, model,X_train,X_test,window_len,test_set,train_set, Y_target, coin):
...
data = get_market_data("bitcoin","BTC")
data = data.drop('Date',1)

separator = int(round(data.index.values.size*0.33))
train_set = data[separator:]
test_set = data[:separator]

X_train = create_inputs(train_set,["BTC"],window_len)
Y_train_btc = create_outputs(train_set, 'BTC',window_len)
X_test = create_inputs(test_set,['BTC'],window_len)
Y_test_btc = create_outputs(test_set, 'BTC',window_len)
...
plot_results(btc_history,btc_model,X_train,X_test,window_len,test_set,train_set,Y_train_btc,"BTC")
